...
function clearMatrix() {
  ...
  if (getPixelBoard.children.length === 0) {
    return;
  }
  const childMatrix = document.getElementsByClassName('line').length;
  for (let index = 0; index < childMatrix; index += 1) {
...
function getSize() {
  ...
  vqv.addEventListener('click', checkInput);
}
